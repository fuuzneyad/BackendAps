...
public class GetFilesHuaweiExecutor
{
  ...
  public static void main(String[] args)
  {
    ...
    GetFilesHuaweiExecutor getFile = new GetFilesHuaweiExecutor(args[0]);
  ...
  public GetFilesHuaweiExecutor(String config)
  {
  ...
  private void GetFiles(String externalParam)
  {
    for (int source = 1; source <= 10; source++)
    {
      ...
      if ((this.ftpProp.isValid()) && (this.ftpProp.isGET_FTP()))
      {
        ...
        String dateArgument = externalParam != null ? "(.*)" + externalParam + "(.*)" : "(.*)";
        	dateArgument ="(.*)";
        ...
        this.ftp.GetSomeHuaweiXml(host, 
          username, 
          password, 
          Integer.parseInt(port), 
          remotePath + subDirDateParam, 
          localPath, 
          regexPattern, 
          dateArgument, 
          this.ftpProp.getMODUL_NAME(), 
          this.ftpProp.isFTP_CHECK_ALREADY_DWL(), 
          regexPrefixNe);
      }
    }
  }
...
private void GetFilesSQS(String externalParam)
  {
    for (int source = 1; source <= 10; source++)
    {
      this.ftpProp = new FTPPropReader(this.prop, source);
