...
public class HuaweiLTEXmlParser extends AbstractParser{
	private ParserPropReader cynapseProp;
	private Map<String, StandardMeasurementModel> modelMap;
	private Map<String, Object> map = new LinkedHashMap<String, Object>();
	private static final Logger logger = Logger.getLogger(HuaweiLTEXmlParser.class);
	
	@SuppressWarnings("unchecked")
	public HuaweiLTEXmlParser(ParserPropReader cynapseProp, AbstractInitiator cynapseInit) {
				...
				public synchronized void startElement(String uri, String localName,String qName, 
					...
					if (this.qName.equalsIgnoreCase("measCollec")){
						...
						if(this.measTime!=null){
							ctx.setDatetimeid(measTime.replace("T", " ").substring(0,measTime.indexOf("+")));
						}
					}else					
		...
		try{
			...
			for (Map.Entry<String, Map<String,String>> entry : tableModel.entrySet()) {
				...
				for (Map.Entry<String, String> entry2 : entry.getValue().entrySet()) {
					...
					String typeTxt = entry2.getValue()!=null && entry2.getValue().toString().length() > 300 ? "TEXT,\n" : "VARCHAR("+((entry2.getValue() == null ? 5 : entry2.getValue().toString().length())+20)+"),\n" ;
					String typeData = entry2.getValue()!=null && isDouble(entry2.getValue()) ? "DOUBLE,\n" : typeTxt; 
