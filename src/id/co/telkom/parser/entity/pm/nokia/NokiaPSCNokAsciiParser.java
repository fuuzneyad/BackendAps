...
public class NokiaPSCNokAsciiParser extends AbstractParser{
	private static final Logger logger = Logger.getLogger(NokiaPSCNokAsciiParser.class);
	private ParserPropReader cynapseProp;
	private Map<String, StandardMeasurementModel> modelMap;
	
	@SuppressWarnings("unchecked")
	public NokiaPSCNokAsciiParser(ParserPropReader cynapseProp,
	          ...
	          if (stringLine.indexOf(",") > 0) {
	            ...
	            if (splitted.length >= 1) {
	              if ((line == 0) && (splitted.length > 3)) {
	                 mdl = (StandardMeasurementModel)this.modelMap.get(splitted[1]);
	                if (mdl != null) {
	                ...
	                if ((line != 1) && !this.cynapseProp.isGENERATE_SCHEMA_MODE()) {
	                  ...
	                  //configurable MO_ID
	                  if(mdl!=null && mdl.getMoIdMapping()!=null && !mdl.getMoIdMapping().equals("") && map!=null){
	    				  String MO="";
	    				  String[] arrS = mdl.getMoIdMapping().split("\\,");
	    				  for (int i=0; i<arrS.length; i++){
	    					  String a=arrS[i].trim();
	    					  Object k = map.get(a);
	    					  if(k!=null)
	    						  if(i!=0)
	    							  MO+="/"+a+"="+k.toString();else
	    								  MO=a+"="+k.toString();  
	    				  }
	    				  ctx.setMo_id(MO);
	    			  }
