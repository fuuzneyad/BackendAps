...
public class HuaweiPSCSgsnParser extends AbstractParser{
	private ParserPropReader cynapseProp;
	private Map<String, StandardMeasurementModel> modelMap;
	
	@SuppressWarnings("unchecked")
	public HuaweiPSCSgsnParser(ParserPropReader cynapseProp, AbstractInitiator cynapseInit) {
...
//			    				  System.out.println("["+x+"]");
			    		  ...
			    		  for (String x:splitted){
			    			  ...
			    			  if(header.get("X"+ctrIdx).equals("Result Time"))	{
			    				  String d = x.split("\\+")[0];
			    				  ctx.setDatetimeid(convertDate(d));
	...
	public void CreateSchemaFromMap(){
		...
		try{
			String location=cynapseProp.getFILE_SCHEMA_LOC()+"HuaweiPSCSgsnSchema.sql";
