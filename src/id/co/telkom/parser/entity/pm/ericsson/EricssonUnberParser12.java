...
public class EricssonUnberParser12 extends AbstractParser{
	private ParserPropReader cynapseProp;
	private Map<String, StandardMeasurementModel> modelMap;
	private static final Logger logger = Logger.getLogger(EricssonUnberParser12.class);
	
	@SuppressWarnings("unchecked")
	public EricssonUnberParser12(ParserPropReader cynapseProp, AbstractInitiator cynapseInit) {
	...
	public synchronized void  ProcessFile(File file,  final LoaderHandlerManager loader,  final Context ctx) throws Exception {
		...
		int tIdx=1;
		int bIdx=1;
		Map<String, Object> aye = new LinkedHashMap<String, Object>();
		    			...
		    			if(!buffer.isEmpty()&&buffer.size()==header.size()){
		    				if(!cynapseProp.isGENERATE_SCHEMA_MODE()){
		    					...
		    					if (std!=null){
				    				...
				    				if(!parserInit.CheckDuplicateRows(ctx.t_name+"|"+ctx.dateTimeid+"|"+ctx.ne_id+"|"+ctx.mo_id)){
				    					loader.onReadyModel(map, ctx);
				    					aye=map;
				    				}else{
				    					System.out.println(ctx.t_name+"|"+ctx.dateTimeid+"|"+ctx.ne_id+"|"+ctx.mo_id+aye);
				    					System.out.println(ctx.t_name+"|"+ctx.dateTimeid+"|"+ctx.ne_id+"|"+ctx.mo_id+map);
				    				}
...
//		    						logger.error("Measurement "+tbl+" have not been mapped yet!!");
		    	...
		    	if(idx==32){//isi
		    		String isi=getChar(strLine);//.replaceAll("&#X", "").replaceAll(";", "").replaceAll("%", "");
