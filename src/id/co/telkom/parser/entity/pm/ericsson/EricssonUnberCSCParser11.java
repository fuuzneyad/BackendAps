...
public class EricssonUnberCSCParser11 extends AbstractParser{
	private ParserPropReader cynapseProp;
	private Map<String, StandardMeasurementModel> modelMap;
	private static final Logger logger = Logger.getLogger(EricssonUnberCSCParser11.class);
	
	@SuppressWarnings("unchecked")
	public EricssonUnberCSCParser11(ParserPropReader cynapseProp, AbstractInitiator cynapseInit) {
...
//		    			ctx.setDatetimeid(convertDate(getChar(strLine).replace("Z", "")));
		    	...
		    	if(idx==16){
		    		if(strLine.contains("[0]")){
		    			bc=getChar(strLine).trim();
		    			if(!bc.equals(""))
		    				ctx.setNe_id(ne+","+bc);
		    		}
		    	}else
		    	if(idx==28){//table & mo
		    		if(strLine.indexOf("<P")>0){
		    			if(!strLine.contains("&#x")){
			    			...
			    			String mo = a.split("\\.").length>1?a.split("\\.")[1]:a;
			    				   ...
			    				   mo=(mo.equals(ne) && !bc.equals(""))?ne+","+bc:mo;//2014-08-04, add BC to MO_ID
			    			ctx.setMo_id(mo);
...
//			    						logger.error("Measurement "+tbl+" have not been mapped yet!!");
	...
	public void CreateSchemaFromMap(){
		try{
			String location=cynapseProp.getFILE_SCHEMA_LOC()+"EricssonCSCSchema.sql";
