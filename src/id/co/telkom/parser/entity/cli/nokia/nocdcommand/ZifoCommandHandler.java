...
public class ZifoCommandHandler  extends AbstractCommandHandler implements MscCommandHandler {
	...
	public ZifoCommandHandler(Parser reader,String command, String params, DataListener listener, String tableMaster, Map<String, ConfiguredHeader[]> headersMap) {
	...
	public void handle(Context ctx) throws IOException {
		...
		while (!isStartExecution && !reader.isEOL() && !reader.isEqual('<')){
			...
			if (sb.toString().startsWith("LOADING PROGRAM")) {				
				...
				return;
			}else if(sb.toString().startsWith("NUMBER STATUS")){
				header=headersMap.get("IFO");
		...
		while(!reader.isEOL()){
				...
				if (header!=null){
					if(reader.isEqual('C')){
						...
						map.put("COMMAND_PARAM", commandParams);
					...
					System.err.println("Skipoooo : "+sb);
					System.exit(1);
				}	
			}
