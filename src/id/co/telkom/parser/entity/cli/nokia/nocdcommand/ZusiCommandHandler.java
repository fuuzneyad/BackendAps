...
public class ZusiCommandHandler extends AbstractCommandHandler implements MscCommandHandler {
	...
	public ZusiCommandHandler(Parser reader, DataListener listener, String command, String params, Map<String, ConfiguredHeader[]> headersMap, String tableName) {
	...
	public void handle(Context ctx)
		...
		while (!isStartExecution && !reader.isEOL() && !reader.isEqual('<')){
			...
			if (sb.toString().startsWith("LOADING PROGRAM")) {				
				...
				return;
			}else if(sb.toString().startsWith("WORKING STATE")){
				reader.skipLines(1);
				header=headersMap.get("USI");
		...
		while(!reader.isEOL()){
				if (header!=null){
					...
					if(sb.toString().trim().startsWith("TOTAL OF   ")||sb.toString().trim().equals("COMMAND EXECUTED")){
						reader.skipEOLs();
						done();
						return;
					}
