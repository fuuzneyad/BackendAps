...
public class ZmviCommandHandler extends AbstractCommandHandler implements MscCommandHandler {
	...
	public ZmviCommandHandler(Parser reader, DataListener listener, String command, String params, Map<String, ConfiguredHeader[]> headersMap, String tableName) {
	...
	public void handle(Context ctx)
		...
		while (!isStartExecution && !reader.isEOL() && !reader.isEqual('<')){
			...
			if (sb.toString().startsWith("LOADING PROGRAM")) {				
				...
				return;
			}else if(sb.toString().contains("SUBSCRIBERS IN VLR")){
				reader.skipEOLs().skipLines(1).skipEOLs();
				header=headersMap.get("MVI");
	...
	private void parse(Map<String, Object> map, Parser reader, ConfiguredHeader[] header) throws IOException {		
		...
		for (int i = 0; i < header.length; i++) {
			...
			{
				if(header[i].getName().equals("ACTUAL") && sb.toString().indexOf("<")<sb.toString().indexOf(")")){
//					System.out.println(sb.toString());
					map.put("ACTUAL", sb.toString().split("\\(")[0].trim());
					map.put("ACTUAL_PERCENT", sb.toString().split("\\(")[1].replace(")", "").trim());
				}else
