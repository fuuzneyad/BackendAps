...
public class GbipStatusLCommandHandler extends AbstractCommandHandler implements MscCommandHandler {
	...
	public GbipStatusLCommandHandler(Parser reader, DataListener listener, String command, String params, Map<String, ConfiguredHeader[]> headersMap) {
	...
	private void Parse(final Parser reader, ConfiguredHeader[]  header) throws IOException{
		...
		for (int i = 0; !isDone() && i <= lastIdx; i++) {
			...
			if(lastIdx==i)
				...
				if(i==0&&sb.toString().startsWith("END")){ 
					done();						
				}
			}
			if(!isDone()){
