...
public class ParserLSTLOCALNRICommandHandler extends AbstractCommandHandler implements MscCommandHandler {
	private final Parser reader;
	private final DataListener listener;
	@SuppressWarnings("unused")
	private Map<String, ConfiguredHeader[]> headersMap = new LinkedHashMap<String, ConfiguredHeader[]>();
	private final String T_NAME;
	private static final Logger logger = Logger.getLogger(ParserLSTLOCALNRICommandHandler.class);
	private Map<String, Object> map = new LinkedHashMap<String, Object>();
	@SuppressWarnings("unused")
	private String Param;
	private boolean isStartExecution;
	public ParserLSTLOCALNRICommandHandler(
			Parser reader, 
	...
	public void handle(Context ctx)
		...
		while (!isStartExecution){
			...
			}else if (sb.toString().startsWith("LOCALNRI table")) {
				reader.skipLines(1);
		...
		while (!reader.isEOF() && !reader.isEqual('(')) {
				...
				if(sb.toString().contains("=")){
					...
					map.put(s.split("=")[0].trim().toUpperCase().replace(" ", "_"), 
							s.split("=")[1].trim());
				}
		}
