...
public class ParserC7GCPCommandHandler extends AbstractCommandHandler implements MscCommandHandler {
	...
	private static final Logger logger = Logger.getLogger(ParserC7GCPCommandHandler.class);
	
	public ParserC7GCPCommandHandler(
			Parser reader, 
		...
		if(isStartExecution){
			...
			while(!reader.isEOF() && !isDone() && !reader.isEqual('<') && !reader.isEqual('E')){
				...
				if(!map.isEmpty()){
					...
					edctx.setEdge_type("CONTROL_PLANE");//MSS-TP
