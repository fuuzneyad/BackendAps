...
public class InitiatorMGRRPCommandHandler extends AbstractCommandHandler implements MscCommandHandler {
	private final Parser reader;
	private final ConfiguredHeader[] header;
	private GlobalBuffer buf;
	private Context ctx;
	
	public InitiatorMGRRPCommandHandler(Parser reader, 
										String command, 
										String params, 
										GlobalBuffer buf,
										Context ctx,
	...
	public void handle(Context ctx)
		...
		while(!isStartExecution && !reader.isEOF() && !reader.isEqual('<')){
			if(reader.isEqual('/')){
				...
				reader.readUntilEOL(sb);
				if(sb.toString().startsWith("MT LOCATION AREA PER")){
		...
		if(isStartExecution){
			...
			while(reader.isEqual(' ') && !reader.isEOF() && !reader.isEqual('<')){
				Parse();
				reader.skipEOL();
			}
		}
	...
	private void Parse() throws IOException{
		...
		//hooray, we get OwnMSRN of this NE
		if(map.get("MSRNS")!=null ){
			String msrn = map.get("MSRNS");
			System.out.println("Set OWN GT ["+msrn+"] to ["+ctx.ne_id+"]");
			buf.setMSRNToVertex(ctx.ne_id, msrn);
		}
	}
}
