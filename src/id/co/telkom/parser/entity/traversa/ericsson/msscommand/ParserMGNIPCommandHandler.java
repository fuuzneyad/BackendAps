...
public class ParserMGNIPCommandHandler extends AbstractCommandHandler implements MscCommandHandler{
	...
	private static final Logger logger = Logger.getLogger(ParserMGNIPCommandHandler.class);
	private Map<String, String> buffer = new LinkedHashMap<String, String>();
	
	public ParserMGNIPCommandHandler(
			Parser reader, 
	...
	public void handle(Context ctx) throws IOException {
		...
		while(!isStartExecution && !reader.isEOF() && !reader.isEqual('<')){
			if(reader.isEqual('/')){
				...
				reader.readUntilEOL(sb);
				if(sb.toString().startsWith("MT NETWORK IDENTITY DATA")){
		...
		while(!isDone() && !reader.isEOF() && !reader.isEqual('<')){
			if(reader.isEqual('S')){//DEV
				reader.readUntilEOL(sb).skipEOL();
				if(sb.indexOf("SNAME") >-1){
