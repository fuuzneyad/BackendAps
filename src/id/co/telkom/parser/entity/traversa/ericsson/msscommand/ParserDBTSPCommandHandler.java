...
public class ParserDBTSPCommandHandler extends AbstractCommandHandler implements MscCommandHandler {
	...
	public ParserDBTSPCommandHandler(
			Parser reader, 
...
//				t_name = null;
				...
				if(sb.indexOf("BLOCK   TAB") >-1){
					...
					ctx.setTableName(T_NAME+"_BLOCK");
				...
				while(!isDone() && !reader.isEOF() && !reader.isEqual('<')){
					if(reader.isEqual('N')){
						...
						if(sb.indexOf("NAME            SETNAME ") >-1){
							...
							check=reader.isEqual('E');//ozan before Equals
							parse(map, reader, header, check);	
							reader.skipEOLs();
							ctx.setTableName(T_NAME+"_BLOCK");
