...
public class ParserZNETRetouchCommandHandler extends AbstractCommandHandler implements MscCommandHandler {
	private final Parser reader;
	private final DataListener listener;
	private final String T_NAME;
	private static final Logger logger = Logger.getLogger(ParserZNETRetouchCommandHandler.class);
	...
	public ParserZNETRetouchCommandHandler(
			//TODO: Generate Topology logic
			Parser reader, 
	...
	protected void parseRoute(Map<String, Object> map, Parser reader, Context context, ConfiguredHeader[] header) throws IOException{
		...
		for (int i = 0; i < header.length-1 && loop; i++) {
			ConfiguredHeader configuredHeader = header[i];
			loop = true;
			boolean isA = i==2 && reader.isEqual('A') ? true : false;
			if(lastHeader==i)
				reader.readUntilEOL(sb).skipEOL();
