...
public class ParserDISPSIGLINKCommandHandler extends AbstractCommandHandler implements CommandHandler {
	...
	public ParserDISPSIGLINKCommandHandler(Parser reader, String command,
		...
		if(!isStartExecution){
			...
			while(!reader.isEOF() && !reader.isEqual('E') && !reader.isEqual('D') && !reader.isEqual('-') && !reader.isEqual('=') && isStartExecution){					
				...
				while(!reader.isEOL()){
					...
					{
						if 	(!map.get(headers[0].getName()).equals("") && !(map.get(headers[10].getName()).equals("") || map.get(headers[13].getName()).equals("")) )//|| map.get(headers[13].getName()).equals("")								
						{
							buffer = map;
						}else
						if 	(map.get(headers[0].getName()).equals("") ){//|| map.get(headers[13].getName()).equals("")
							if (!map.get(headers[10].getName()).equals(""))
								buffer.put(headers[10].getName(), buffer.get(headers[10].getName())+" "+map.get(headers[10].getName()));
							if (!map.get(headers[13].getName()).equals(""))
								buffer.put(headers[13].getName(), buffer.get(headers[13].getName())+"\n"+map.get(headers[13].getName()));
							map=buffer;
							buffer=map;
						}									
					...
					//insert when done
						if (buffer.get(headers[0].getName())!=null )
	...
	private void parse(Map<String, Object> map, Parser reader, ConfiguredHeader[] header) throws IOException {		
		try{
			...
			if (!(value.contains("---"))){//|| value.contains("===")|| value.contains("Net name") || value.contains("identifier | le name")  || value.contains("|          |                |")
				int start=0;
