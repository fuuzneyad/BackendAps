...
public class ParserDISPSIGDPCommandHandler extends AbstractCommandHandler implements CommandHandler {
	...
	public ParserDISPSIGDPCommandHandler(Parser reader, String command,
				...
				while(!reader.isEOL()){
					...
					parse(map, reader, headers);	
						if (map.get(headers[0].getName())!=null )//the line
							{
								if 	(!map.get(headers[0].getName()).equals("") && !map.get(headers[11].getName()).equals("") )								
								{
									buffer = map;
								}else
								if 	(map.get(headers[0].getName()).equals("") && !map.get(headers[11].getName()).equals("") ){
									buffer.put(headers[11].getName(), buffer.get(headers[11].getName())+"\n"+map.get(headers[11].getName()));
									map=buffer;
									buffer=map;
								}									
