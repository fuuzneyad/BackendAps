...
public class ImocParserXlsBTSPerformance extends AbstractParser{
	
	private Map<String, StandardMeasurementModel> modelMap ;
	
	@SuppressWarnings("unchecked")
	public ImocParserXlsBTSPerformance(ParserPropReader cynapseProp, AbstractInitiator parserInit) {
	...
	protected void ProcessFile(File file, LoaderHandlerManager loader, Context ctx) throws Exception {
		...
		if(mdl!=null){
			...
			if(file.getName().endsWith("xlsx")){
				...
				XSSFSheet sheet  = wbx.getSheetAt(1);
			             ...
			             while( cells.hasNext() ) {
			            	 ...
			            	 if(iRow==0){
			            		...
			            		if(cynapseProp.isGENERATE_SCHEMA_MODE()){
			            			PutModel(mdl.getTableName(), counterName, "0");
			            		}
			            	 } else{
			            		 if(cynapseProp.isGENERATE_SCHEMA_MODE())
			            			 break;
			            		 ...
			            		 if(headerName!=null){
			            		 	if(headerName.equals("DATE")){
			            		 		if(isInteger(isi.toString()))
			            		 			ctx.setDatetimeid(convertIntToDate(convertIntToDate(isi.toString())));
			            		 		else
...
//	}
	...
	private static String convertIntToDate(String s){
		int i = 0;
		try{
